...
local poll_timeout = 0 -- Async requires no wait for poll.
...
local function query( callback, str, ... )
	reconnectIfDisconnected( )
	
	str = joinArguments( str, ... )

	local function query_handler( query_handle ) 
		if query_handle then
			...
			callback( false, "Unable to allocate query handle in pool" )
			return 
		end
	...
	connection:query( query_handler, str )
end
...
function query_free( callback, str, ... )
...
function query_assoc( callback, str, ... )
	if isSourceResourceNotAllowed() then
		return false
	end

	local function query_handler( query_handle_index, error )
	...
	query( query_handler, str, ... )
end

function query_assoc_single( callback, str, ... )
	if isSourceResourceNotAllowed() then
		return false
	end
...
function query_insertid( callback, str, ... )
	if isSourceResourceNotAllowed() then
		return false
	end
...
function query_affected_rows( callback, str, ... )
	if isSourceResourceNotAllowed() then
		return false
	end
